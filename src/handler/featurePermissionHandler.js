...
const {
  createNewKegiatanFeature,
} = require("../feature/createNewKegiatanFeature");
const { refIdValidator } = require("../util/validator");
const { getUserPermissionLevel } = require("../util/getUserPermissionLevel");
const {
  getMinimumFeaturePermission,
} = require("../util/getMinimumFeaturePermission");
...
const postFeaturePermissionHandler = async (req, res) => {
  ...
  if (!isFeatureIdValid) {
    res
      .status(404)
      .json({ errorMessage: "Feature not found or invalid feature id used." });
    return;
  }
  ...
  try {
    const minimumFeaturePermission = await getMinimumFeaturePermission(
      featureId
    );
    const userPermissionLevel = await getUserPermissionLevel(req.email);

    if (userPermissionLevel < minimumFeaturePermission) {
      await generateErrorEmail(
        `${req.email} trying to access ${req.originalUrl}`
      );
      res
        .status(403)
        .json({
          errorMessage:
            "FORBIDDEN. You do not have required permission to use this feature.",
        });
      return;
    }
...
const getFeaturePermissionHandler = async (req, res) => {
  ...
  try {
    if (!validateFeatureId(featureId))
      throw new CustomError("Feature Id Invalid!");
  } catch (e) {
  ...
  try {
    ...
    if (featurePermissionLevel > userPermissionLevel)
