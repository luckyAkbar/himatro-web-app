...
const insertKehadiranRecord = async (queryParams, bodyRequest) => {
  ...
  const query = 'UPDATE absensi SET keterangan = $1, alasan_izin = $2, waktu_pengisian = $3 WHERE npm = $4 AND referensi_id = $5';
  const params = [keterangan, alasan, waktuPengisian, npm, absentId];

  try {
    await testQuery(query, params);
  } catch (e) {
    throw new CustomError('Maaf, server gagal memasukan data anda ke dalam daftar absensi.', 500);
  }
};

const checkIfUserAlreadyFilledAbsentForm = async (NPM, absentId) => {
  const query = 'SELECT keterangan FROM absensi WHERE npm = $1 AND referensi_id = $2';
  ...
  try {
    ...
    throw new CustomError('Anda sudah mengisi form ini, dan tidak bisa mengubahnya.');
  }
};
...
const validateAbsentFormWriteableStatusFromDate = ({ tanggalPelaksanaan, tanggalBerakhir, namaKegiatan }) => {
  const now = new Date();
  const stillClosedErrorMsg = `Form absen ${namaKegiatan} akan dibuka pada ${formatDateToStandartFormat(tanggalPelaksanaan)}`;
  const alreadyClosedErrorMsg = `Form absen untuk ${namaKegiatan} sudah ditutup pada ${formatDateToStandartFormat(tanggalBerakhir)}`;
...
const checkIsNPMExistsOnPengurusTable = async (NPM) => {
  ...
  try {
    ...
    throw new CustomError('Maaf, NPM anda tidak ditemukan sebagai pengurus Himatro.', 404);
  }
};
...
const checkIsNPMExistsOnAbsentEntry = async (absentId, NPM) => {
  ...
  try {
    ...
    throw new CustomError('Maaf, NPM anda tidak ditemukan di daftar absensi. Mohon periksa NPM anda.', 404);
  }
};
